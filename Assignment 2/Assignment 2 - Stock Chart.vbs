...
Sub TotalStockVolume():
   ...
   ' For Each ws In Worksheets
        ...
        ws.Range("I1").Value = "Total Volume"
        ...
        LastRow = ws.Cells(Rows.Count, 1).End(xlUp).Row
            For i = 2 To LastRow   
                ...
                If ws.Cells(i + 1, 1).Value <> ws.Cells(i, 1).Value Then
                    ...
                    ws.Range("H" & SummaryRow) = ws.Cells(i, 1).Value
                    ws.Range("I" & SummaryRow) = TotalVolume + Cells(i, 7).Value
            ...
            Next i
        'Apply formatting
        ws.Range("C2:G" & LastRow & ", " & "I2:I" & LastRow).Style = "Currency"
        ws.Range("B2:B" & LastRow).Style = 
        ws.Columns("A:I").AutoFit
   ' Next ws
    'Loop to next worksheet and repeat logic
End Sub
...
Sub TotalStockVolume():
    ...
    For Each ws In Worksheets
        ...
        ws.Range("T2").Value = ws.Application.WorksheetFunction.Max(Range("O:O"))
        For j = 2 To LastRow
            If ws.Range("T2").Value = ws.Cells(j, 15).Value Then
                ws.Range("S2").Value = ws.Cells(j, 1).Value
            End If
            Exit For
        Next j
    'Apply formatting
